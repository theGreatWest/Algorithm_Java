...
//        System.out.println(sb);
        ...
        while (!q.isEmpty()) {
            ...
            for (int next : new int[]{current - 1, current + 1, current * 2}) {
                ...
                if (depth[next] == -1) { // 아직 방문하지 않은 노드
                    ...
                    q.offer(next);
                } else if (depth[next] == depth[current] + 1) prevNode[next].add(current); // 이미 방문한 노드인 경우, 다음 노드의 깊이가 현재 노드의 깊이 + 1과 같을 때, 바로 이전 노드라고 판단할 수 있다.
            }
        }
    }
    ...
    static void bfs() {
        ...
        while (!q.isEmpty()) {
            ...
            if (currNum == k) {
                if (minSec == 0) minSec = currDepth;
                if (minSec == currDepth) way++;
            }
